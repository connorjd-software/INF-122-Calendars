...
class Calendar {
    ...
    public void displayOptions() {
        ...
        System.out.println("b = Back | a = Add event | e = Edit Event | t = Toggle Privacy | s = Share With | v = Change View");
    }
    ...
    public void handleUserOptions() throws IOException {
        ...
        commands.put("a", new AddEventCommand(this));
        commands.put("e", new EditEventCommand());
        commands.put("t", new TogglePrivacyCommand(this));
        commands.put("s", new ShareCalendarCommand(this, "exampleUsername"));
        commands.put("v", new ChangeViewCommand(this));
    ...
    static class BackCommand implements Command {
        @Override
        public void execute() {
    ...
    static class AddEventCommand implements Command {
        private Calendar calendar;

        public AddEventCommand(Calendar calendar) {
        ...
        public void execute() {
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
                ...
                calendar.addEvent(new Event(eventName, startTime, endTime, description));
                System.out.println("Event added.");
            } catch (DateTimeParseException e) {
                System.out.println("Error parsing date-time input. Make sure to use the format YYYY-MM-DDTHH:MM.");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    // Command to edit an event
    static class EditEventCommand implements Command {
        @Override
        public void execute() {
    ...
    // Command to toggle calendar privacy
    static class TogglePrivacyCommand implements Command {
        private Calendar calendar;

        public TogglePrivacyCommand(Calendar calendar) {
            this.calendar = calendar;
        }

        @Override
        public void execute() {
            String visibility = calendar.isPrivate ? "public" : "private";
            calendar.toggleIsPrivate();
            System.out.println("Changed calendar visibility to: " + visibility);
        }
    }

    // Command to share the calendar with another user
    static class ShareCalendarCommand implements Command {
        private Calendar calendar;
        private String username;

        public ShareCalendarCommand(Calendar calendar, String username) {
            this.calendar = calendar;
        ...
        @Override
        public void execute() {
            calendar.shareWith(username);
        }
    }

    // Command to change calendar view (Day, Week, Month, Year)
    static class ChangeViewCommand implements Command {
        private Calendar calendar;

        public ChangeViewCommand(Calendar calendar) {
            this.calendar = calendar;
        }
